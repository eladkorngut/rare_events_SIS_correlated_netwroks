...
def fluctuation_run_assortative(Alpha,bank,outfile,infile,runs,Num_inf,network_number,Beta):
    ...
    for run_loop_counter in range(runs):
        ...
        r = np.random.uniform(0, 1, (bank, 2))
        nx.set_node_attributes(G,False,'infected')
        Rates = netinithomo.inatlize_inf_assortative(G,Num_inf,G.number_of_nodes(),Alpha,Beta)
        net_num = []
        net_num.append(network_number)
...
def actasmain():
    ...
    prog = 'bda' #can be either 'i' for the inatilization and reaching eq state or 'r' for running and recording fluc
    Lam = 1.6
    Time_limit = 200
    ...
    d1_in, d1_out, d2_in, d2_out = int(k * (1 - eps_din)), int(k * (1 - eps_dout)), int(k * (1 + eps_din)), int(
        k * (1 + eps_dout))
    G, graph_degrees = rand_networks_assortative.configuration_model_undirected_graph_mulit_type(k, eps_din, N, prog,
                                                                                                 correlation_factor)
    ...
    fluctuation_run_assortative(Alpha,bank,outfile,infile,1,Num_inf,0,Beta)
        ...
        actasmain()
         ...
         if sys.argv[1]=='i':
            ...
            # Run and record number of infected for bimodal degree graph with infection process 1-(1-lam)^m
             ...
             # Extinction for a gamma distrubtion network
             fluctuation_run_extinction_DiGraph(float(sys.argv[2]), int(sys.argv[3]), sys.argv[4], sys.argv[5],
                                                int(sys.argv[6]),int(sys.argv[7]),int(sys.argv[8]),float(sys.argv[9]))
         elif sys.argv[1] == 'bda':
             # Run a undirected network with assortative mixing
             fluctuation_run_assortative(float(sys.argv[2]),int(sys.argv[3]),sys.argv[4], sys.argv[5],int(sys.argv[6]),int(sys.argv[7]),int(sys.argv[8]),float(sys.argv[9]))

    end_time = time.time()
    run_time = end_time - start_time
    np.save('run_time_gillespie.npy', run_time)
